...
For this year's competition, we focus on test prioritization.
...
| Sebastiano     | Panichella    | University of Bern & AI4I The Itialian Institute of Artificial Intelligence for Industry | sebastiano.panichella@unibe.ch    |

## Competition Venues and Dates
### ICST 2026, Daejeon, Republic of Korea
| **Event**                          | **Date (AoE)**   |
|------------------------------------|------------------|
| Tool Submission                    | 1.03.2026 (AoE)  |
| Tool Paper Submission              | 15.03.2026 (AoE) |
| Camera-ready Tool Paper Submission | 22.03.2026 (AoE) |

### SBFT 2026, Rio de Janeiro, Brazil
| **Event**               | **Date (AoE)**   |
|-------------------------|------------------|
| Tool Submission         | 30.11.2025 (AoE) |
| Notification            | 22.12.2025 (AoE) |
| Camera Ready            | 26.01.2026 (AoE) |

## Test Prioritization
*Test Prioritization* is the process of ordering a selection of test cases from the test suite.
...
## Goal
The goal is to develop cost-effective novel test prioritization tools.
The participants of the tool competition submit a test prioritizer for simulation-based tests of SDCs.
...
To make the evaluation of the tools coherent, the competitors have to implement a gRPC [^4] interface specified in the `tools/prioritizers/interface_2026.proto` file.
...
Below you see figures illustrating the overall set up.
...
sequenceDiagram
    ...
    ToolX -->>- Evaluator: return prioritized order
```

### Test Case Definition
For the competition, we define a single test case as a sequence of road points.
When you look at the `interface_2026.proto` file, you will find the following definitions:
```protobuf
...
message SDCTestCase {
...
There are no major technical limitations for the implementation of a test prioritization approach.
The competitors have only to implement the provided interfaces (`interface_2026.proto`) and ensure that their tool works inside a Docker container.

The participants have to generate first the interface stubs using the `protoc` compiler to generate the code based on the interface specification in the `interface_2026.proto` file.

In `tools/prioritizers/sample_tool` is a sample implementation of a trivial test prioritizer.
...
```bash
cd tools/prioritizers/sample_tool
...
This is a clear indication the the gRPC interfaces are working properly, which is crucial for the competition's evaluation.
...
- *Average Fault Detection Rate (APFDR)*: The average number of test failures we observe when running the ordered test cases. The earlier the faults appear in the order the better the prioritization is.

> For a clear description of the evaluation metrics, refer to the `MetricEvaluator` class of the prioritization evaluator!
...
The competitors have to make sure that their tools will work on the above mentioned specifications.

## Tool Submission
There are two ways to submit a tool:

### Open Source (preferred)
We ask the competitors to submit their tool by opening a Pull Request to this repository.
...
```text
TO: christian.birchler@unibe.ch
SUBJECT: [2026 SDC Tool Competition] Submission <TOOL NAME>
```
